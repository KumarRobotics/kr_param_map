...
class StructMapGenerator {
 ...
 public:
  ...
  void initParams(param_env::GridMapParams& mpa) {
    ...
    mpa.basic_mp_.max_range_ =
        mpa.basic_mp_.map_origin_ + mpa.basic_mp_.map_size_;
    mpa.basic_mp_.map_volume_ = mpa.basic_mp_.map_size_(0) *
                                mpa.basic_mp_.map_size_(1) *
                                mpa.basic_mp_.map_size_(2);
  ...
  void generate() {
    ...
    std::cout << "+++ cylinders  : "
              << 100 * float(cylinder_grids) / float(all_grids)
              << "%           +++" << std::endl;
    std::cout << "+++ circles    : "
              << 100 * float(circle_grids) / float(all_grids)
              << "%           +++" << std::endl;
    std::cout << "+++ gates      : "
              << 100 * float(gate_grids) / float(all_grids) << "%           +++"
              << std::endl;
    std::cout << "+++ ellipsoids : "
              << 100 * float(ellip_grids) / float(all_grids)
              << "%           +++" << std::endl;
    std::cout << "+++ polytopes  : "
